...
public class NewsController extends ABasicController{
    ...
    @GetMapping(value = "/list_news", produces = MediaType.APPLICATION_JSON_VALUE)
    public ApiMessageDto<ResponseListObj<NewsDtoListClient>> listNews(NewsCriteria newsCriteria, Pageable pageable){
        ApiMessageDto<ResponseListObj<NewsDtoListClient>> apiMessageDto = new ApiMessageDto<>();
        Page<News> newsPage = newsRepository.findAll(newsCriteria.getSpecificationFe(), pageable);
        ResponseListObj<NewsDtoListClient> newsList = new ResponseListObj<>();
        newsList.setData(newsMapper.fromEntityListToDtoListClient(newsPage.getContent()));
    ...
    @GetMapping(value = "/get_news/{id}", produces = MediaType.APPLICATION_JSON_VALUE)
    public ApiMessageDto<NewsDto> listNews(@PathVariable("id") Long id){
        ApiMessageDto<NewsDto> apiMessageDto = new ApiMessageDto<>();

        News newsPage = newsRepository.findById(id).orElse(null);
        apiMessageDto.setData(newsMapper.fromEntityToDto(newsPage));
        apiMessageDto.setMessage("List news success");
        return apiMessageDto;
    }
    ...
    public ApiMessageDto<ResponseListObj<NewsAdminDto>> list(NewsCriteria newsCriteria, Pageable pageable) {
        ...
        ApiMessageDto<ResponseListObj<NewsAdminDto>> responseListObjApiMessageDto = new ApiMessageDto<>();

        Page<News> list = newsRepository.findAll(newsCriteria.getSpecification(), pageable);
        ResponseListObj<NewsAdminDto> responseListObj = new ResponseListObj<>();
    ...
    public ApiMessageDto<NewsAdminDto> get(@PathVariable("id") Long id){
        ...
        ApiMessageDto<NewsAdminDto> result = new ApiMessageDto<>();
